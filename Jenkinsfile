...
void db_migrate(namespace='staging') {
  if (namespace=='staging') {
    sh "kubectl run optimus-migrate --image=${IMAGE_REPO}/${IMAGE_NAME}:staging-90 --attach=true --rm=true --restart='Never' --env='RAILS_ENV=${STAGING_ENV}' --env='OPTIMUS_DB_URL=${STAGING_DB}' --env='SENTRY_DSN=${SENTRY_DSN}' --command -- env --namespace ${namespace}  --kubeconfig=/home/devops/.kube/jenkins-k8s-config"
